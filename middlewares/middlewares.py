...
from utils.db_utils import get_user, get_user_route
...
def check_signature():
    ...
    # except SignRequestException as e:
...
def get_route():
    ...
    if not backend:
        abort(404)
    g.route = {'id': backend.idx, 'method': backend.method, 'path': backend.url, 'hosts': backend.hosts,
               'protocol': backend.protocol, 'seconds': backend.seconds, 'limits': backend.limits}
